...
module.exports = function(args, finished) {
  ...
  var headingRef = sourceId.split('BlackPear-')[1];
  var id = headingRef.split('_')[1];

  console.log('*** getHeadingDetail for ' + heading + ': ' + id);
  ...
  getPatientsByNHSNumber(patientId, authCredentials, session, function(error) {
    if (error) {
      ...
      getPatientResources(patientId, authCredentials, session, function(error) {
        ...
        var results = getHeadingDetail(patientId, heading, id, 'pulsetile', session);
        finished({
          responseFrom: 'blackpear_service',
          results: results
        });
